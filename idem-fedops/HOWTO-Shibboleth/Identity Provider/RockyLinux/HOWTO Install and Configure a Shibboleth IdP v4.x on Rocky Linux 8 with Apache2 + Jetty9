 ...
 * mysql-connector-j (if JPAStorageService is used)
...
2. Download and Extract Jetty:
   ...
   wget https://repo1.maven.org/maven2/org/eclipse/jetty/jetty-distribution/9.4.50.v20221201/jetty-distribution-9.4.50.v20221201.tar.gz
   
   tar xzvf jetty-distribution-9.4.50.v20221201.tar.gz
   ```

3. Create the `jetty-src` folder as a symbolic link. It will be useful for future Jetty updates:
   * `ln -nsf jetty-distribution-9.4.50.v20221201 jetty-src`
...
> :warning: **DO IT BECAUSE IT IS IMPORTANT**
>
...
2. Install required packages:
   ...
   * `rpm -Uvh mysql80-community-release-el8.rpm`
   * `dnf --enablerepo=mysql-connectors-community install mysql-connector-j`
...
7. Rebuild IdP with the needed libraries:
   * `cd /opt/shibboleth-idp`
   * `ln -s /usr/share/java/mysql-connector-j.jar edit-webapp/WEB-INF/lib`
   * `bin/build.sh`
   ...
   2. IDEM MDS (legacy):
      ...
      * `vim /opt/shibboleth-idp/conf/metadata-providers.xml`
        ...
        <MetadataProvider
           ...
           <!-- 
               Consume only SP in the metadata aggregate for Shibboleth IdP 4.0:
           -->
           <!--
